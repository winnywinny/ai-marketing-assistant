...
# ==========================================
# 1. IoT 赛博工业视觉配置 (高对比度 v4.1)
# ==========================================
st.set_page_config(page_title="IoT 全域智慧中枢 v4.1", page_icon="📡", layout="wide")

# 自定义深色 IoT 主题 CSS - 重点优化了文字颜色
st.markdown("""
<style>
    /* 整体背景 */
    .stApp {
        background: radial-gradient(circle, #0a0e14 0%, #020202 100%);
        color: #FFFFFF !important; /* 强制所有主文字为纯白 */
    ...
    section[data-testid="stSidebar"] .css-17l244g, section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] label {
        color: #00f2ff !important; /* 侧边栏标签改为电光青 */
        font-weight: bold;
    }
    ...
    div[data-testid="metric-container"] {
        ...
        border: 2px solid #00f2ff; /* 加粗边框线 */
        border-radius: 6px;
        padding: 20px;
        box-shadow: 0 0 20px rgba(0, 242, 255, 0.2);
    }
    
    /* 指标标题(Label) 改为明亮的黄色，增加辨识度 */
    ...
    .stTabs [aria-selected="true"] {
        background: linear-gradient(135deg, #00f2ff 0%, #a200ff 100%) !important;
    }
    .stTabs [aria-selected="true"] p {
        color: #000000 !important; /* 选中时黑字，保证对比度 */
        font-weight: 900;
    }
    
    /* 底部状态栏字颜色加亮 */
    .stCaption {
        color: #00ff41 !important; /* 改为亮绿色 */
        font-weight: bold;
...
# ==========================================
if 'init_pulse' not in st.session_state:
    with st.status("📡 正在建立高带宽卫星链路...", expanded=True) as s:
        st.write(">> 注入端到端量子加密协议...")
        time.sleep(0.3)
        st.write(">> 扫描 RAG 知识库索引...")
        st.session_state.init_pulse = True
        s.update(label="IoT 智慧中枢上线成功", state="complete")
...
def retrieve_knowledge(query, vectorizer, tfidf_matrix, paragraphs, top_k=2):
    ...
    similarities = cosine_similarity(query_vec, tfidf_matrix).flatten()
    top_indices = similarities.argsort()[-top_k:][::-1]
    return [paragraphs[idx] for idx in top_indices if similarities[idx] > 0.05]
...
st.sidebar.markdown("### 📊 节点协议配置")
selected_platform = st.sidebar.selectbox("接入协议端口", list(logo_map.keys()) + ["🛠️ 自定义协议"])

if selected_platform in logo_map:
    st.sidebar.image(logo_map[selected_platform], width=150)

st.sidebar.markdown("---")
st.sidebar.markdown("### 🖥️ 系统资源监控")
st.sidebar.progress(0.4, text="CPU 负载: 40%")
st.sidebar.progress(0.8, text="网速下行: 22ms")

# RAG 固件上传
st.sidebar.markdown("---")
st.sidebar.header("📚 知识库注入")
knowledge_file = st.sidebar.file_uploader("注入运营SOP固件 (TXT)", type=["txt"])
rag_enabled = False
paragraphs, tfidf_matrix, vectorizer = [], None, None
...
def load_data(platform):
    dates = pd.date_range(start='2024-01-01', periods=1000, freq='h')
    df = pd.DataFrame({
        'Category': np.random.choice(['核心数码', '生物制剂', '精密服饰', '合成食品'], 1000),
        'Power_Flux': np.random.uniform(100, 5000, 1000),
        'Timestamp': np.random.choice(dates, 1000),
        'CityNode': np.random.choice(['北京', '上海', '广州', '深圳', '杭州'], 1000),
    })
    df['Timestamp'] = pd.to_datetime(df['Timestamp'])
    df['Date'] = df['Timestamp'].dt.date
    return df
...
# 5. 主页面布局
# ==========================================
st.markdown(f"# 🛰️ {selected_platform} 数据中心 · 实时监控")

# 顶部指标 (颜色优化：黄、青、绿)
m1, m2, m3, m4 = st.columns(4)
real_gmv = df['Power_Flux'].sum()
m1.metric("⚡ POWER FLUX (GMV)", f"Ξ {real_gmv/1000:.1f}K", "+12.5%")
m2.metric("🛰️ NODES (订单量)", f"{len(df)} Pkts", "-3.2%")
m3.metric("👥 ACTIVE UV (访客)", f"{len(df)*22}", "+5.4%")
m4.metric("⚙️ SECURITY LEVEL", "A-MAX")

tab1, tab2, tab3 = st.tabs(["[ 📡 核心光谱 ]", "[ 🔋 品类分布 ]", "[ 🧠 AI 诊断 ]"])

with tab1:
    col_l, col_r = st.columns([7, 3])
    with col_l:
        daily = df.groupby('Date')['Power_Flux'].sum().reset_index()
        fig_wave = px.area(daily, x='Date', y='Power_Flux', title="GMV 能量波动波形图")
        # 优化图表轴颜色，确保可见
        fig_wave.update_layout(
            template="plotly_dark",
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            xaxis=dict(tickfont=dict(color='#00f2ff'), title_font=dict(color='#FFD700')),
            yaxis=dict(tickfont=dict(color='#00f2ff'), title_font=dict(color='#FFD700'))
        )
        fig_wave.update_traces(line_color='#00f2ff', fillcolor='rgba(0, 242, 255, 0.2)')
        st.plotly_chart(fig_wave, use_container_width=True)
    with col_r:
        st.markdown("### 📜 系统实时日志")
        logs = [f"> [{datetime.now().strftime('%H:%M:%S')}] 🟢 节点同步正常", 
                f"> [{datetime.now().strftime('%H:%M:%S')}] 🔵 RAG 检索链路激活",
                f"> [{datetime.now().strftime('%H:%M:%S')}] 🟡 发现高价值流量突发"]
...
with tab2:
    ...
    with c1:
        cat_data = df.groupby('Category')['Power_Flux'].sum().reset_index()
        fig_pie = px.pie(cat_data, values='Power_Flux', names='Category', hole=0.6, title="品类能效占比")
        fig_pie.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig_pie, use_container_width=True)
    with c2:
        fig_bar = px.bar(df.groupby('CityNode')['Power_Flux'].count().reset_index(), x='CityNode', y='Power_Flux', title="节点地理分布密度")
        fig_bar.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)')
        fig_bar.update_traces(marker_color='#a200ff')
        st.plotly_chart(fig_bar, use_container_width=True)

with tab3:
    st.markdown("### 🧠 深度神经诊断报告")
    if st.button("⚡ 立即启动全量诊断", type="primary"):
        with st.status("正在解析多维数据...", expanded=True) as status:
            context = ""
            if rag_enabled:
                hits = retrieve_knowledge("运营现状", vectorizer, tfidf_matrix, paragraphs)
                context = "\n".join(hits)
                st.write("✅ 内部固件数据已匹配...")
            
            prompt = f"作为高级AI，分析{selected_platform}平台数据：GMV {real_gmv}。结合参考：{context}。"
            ...
            try:
                ...
                st.markdown(f"<div class='terminal-box' style='color:#FFFFFF;'>{response.choices[0].message.content}</div>", unsafe_allow_html=True)
                status.update(label="诊断完成", state="complete")
            except Exception as e:
                st.error(f"AI 通讯中断: {e}")
...
c1, c2, c3, c4 = st.columns(4)
c1.caption("📶 信号强度: 满格")
c2.caption("🔒 加密协议: 量子级 AES-512")
c3.caption("🚀 诊断引擎: Gemini-Flash-Ultra")
c4.caption(f"📅 节点日期: {datetime.now().strftime('%Y-%m-%d')}")
