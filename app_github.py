...
# 1. 视觉增强：高对比度赛博大屏 UI
# ==========================================
st.set_page_config(page_title="IoT 全域电商智慧中枢 v5.0", page_icon="📡", layout="wide")

st.markdown("""
<style>
    /* 全局背景：纯黑到深蓝的深度渐变 */
    .stApp {
        background: linear-gradient(180deg, #050505 0%, #0a0e14 100%);
        color: #FFFFFF; /* 全局文字强制纯白 */
    }
    
    /* 修正灰色字问题：强制侧边栏和正文标签为高亮色 */
...
def get_sys_metrics():
    return {
        "load": np.random.randint(10, 45),
        "temp": np.random.randint(30, 55),
        "uptime": "99.98%"
    }
...
def process_rag(text):
    chunks = [p.strip() for p in text.split('\n') if len(p.strip()) > 5]
    return chunks, [" ".join(jieba.cut(c)) for c in chunks]

# ==========================================
# 4. 侧边栏：多平台 Logo 与控制
# ==========================================
st.sidebar.markdown("<h2 style='color:#00f2ff;'>📡 控制中枢</h2>", unsafe_allow_html=True)

logo_map = {
    "淘宝": "https://img.alicdn.com/tfs/TB1_uT8a5ERMeJjSspiXXbZLFXa-143-59.png",
    "天猫": "https://img.alicdn.com/tfs/TB1T795PXXXXXcXXXXXXXXXXXX-143-59.png",
    "京东": "https://misc.360buyimg.com/lib/img/e/logo-201305-b.png",
    "拼多多": "https://img-ptp.pinduoduo.com/show/logo/pinduoduo_logo.png"
}

platform = st.sidebar.selectbox("切换监测节点", list(logo_map.keys()) + ["自定义注入"])

# 显示 Logo
if platform in logo_map:
    st.sidebar.image(logo_map[platform], width=130)

st.sidebar.markdown("---")
# 知识库注入
knowledge_file = st.sidebar.file_uploader("🚀 注入 SOP 固件 (RAG)", type=["txt"])
rag_ready = False
if knowledge_file:
    chunks, corpus = process_rag(knowledge_file.read().decode())
    vec = TfidfVectorizer().fit(corpus)
    matrix = vec.transform(corpus)
    rag_ready = True
    st.sidebar.success("固件同步成功")

# 系统指标
metrics = get_sys_metrics()
st.sidebar.metric("SYSTEM LOAD", f"{metrics['load']}%", "Stable")
st.sidebar.metric("NODE TEMP", f"{metrics['temp']}°C", "-2°C")

# ==========================================
# 5. 主页面：可视化大屏布局
# ==========================================
st.markdown(f"<h1 style='color:#00f2ff; text-align:center;'>{platform} 全域数据监控大屏</h1>", unsafe_allow_html=True)
...
df_size = 1200
df = pd.DataFrame({
    'Amount': np.random.uniform(50, 2000, df_size),
    'Date': pd.date_range(start='2024-01-01', periods=df_size, freq='h'),
    'Cat': np.random.choice(['数码', '美妆', '服饰', '食品'], df_size),
    'City': np.random.choice(['北京', '上海', '广州', '深圳'], df_size)
})

c1.metric("NETWORK GMV", f"¥{(df['Amount'].sum()/10000):.2f}W")
c2.metric("ORDER PULSE", f"{df_size} Pkts")
c3.metric("U-ID SYNC", f"{df_size*15} UV")
c4.metric("AOV DEPTH", f"¥{df['Amount'].mean():.1f}")

st.markdown("---")

t1, t2, t3 = st.tabs(["📈 实时波谱", "📊 节点拓扑", "🧠 AI 决策"])
...
with t1:
    ...
    with col_l:
        ...
        fig = px.area(daily, x='Date', y='Amount', title="实时能量波动图")
        fig.update_traces(line_color='#00f2ff', fillcolor='rgba(0, 242, 255, 0.2)')
        fig.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig, use_container_width=True)
    with col_r:
        st.markdown("### 📜 节点日志")
        logs = [f"[{datetime.now().strftime('%H:%M:%S')}] 接入协议: {platform}", 
                f"[{datetime.now().strftime('%H:%M:%S')}] 数据包校验成功",
                f"[{datetime.now().strftime('%H:%M:%S')}] RAG 神经元已激活",
                f"[{datetime.now().strftime('%H:%M:%S')}] 安全等级: 最高"]
...
with t2:
    ...
    with cc1:
        # 环形图参考图片样式
        cat_df = df.groupby('Cat')['Amount'].sum().reset_index()
        fig_pie = px.pie(cat_df, values='Amount', names='Cat', hole=0.6, title="品类资源占比")
        fig_pie.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig_pie, use_container_width=True)
    with cc2:
        # 柱状图展示城市
        city_df = df.groupby('City')['Amount'].count().reset_index()
        fig_bar = px.bar(city_df, x='City', y='Amount', color='Amount', title="区域节点密度", color_continuous_scale='Blues')
...
with t3:
    st.markdown("### 🧠 增强型逻辑内核")
    if not rag_ready:
        st.info("💡 请在左侧注入 SOP 固件以开启【专家模式】诊断。")
    
    if st.button("🚀 启动全域 AI 诊断", type="primary"):
        with st.status("正在扫描链路数据...", expanded=True) as status:
            time.sleep(1)
            # 这里调用 OpenAI 逻辑
            summary = f"节点：{platform}，当前 GMV：{df['Amount'].sum():.0f}。"
            try:
                # 模拟输出
                res = "### 诊断建议：\n1. **流量异常**：发现上海节点流量激增，建议增加带宽。\n2. **品类优化**：数码品类能耗过高，请核对库存协议。"
                st.markdown(f"<div style='background:rgba(0,242,255,0.1); padding:20px; border-radius:10px; border:1px solid #00f2ff; color:#E0E0E0;'>{res}</div>", unsafe_allow_html=True)
                status.update(label="诊断完成", state="complete")
            except Exception as e:
                st.error(f"内核异常: {e}")

# 底部状态栏
st.markdown("<div style='text-align:center; color:#00f2ff; padding:20px;'>📶 信号强度: 极强 | 🔒 加密协议: AES-256 | 🚀 驱动引擎: Gemini 3 Flash</div>", unsafe_allow_html=True)
