...
st.markdown("支持批量上传原始订单流水，系统将自动清洗、聚合数据，并生成可视化看板与 AI 专家诊断。")

# 2. 初始化大模型客户端 (根据你上传的 CSV 文件，为你专属修正了节点 URL)
api_key = os.environ.get("OPENAI_API_KEY", "sk-cdc57fd44a394cea8ec25dd4ad2512f7") # 你的真实 Key
client = OpenAI(
    api_key=api_key, 
    base_url="https://ws-vvr85dv3ndbxfxtu.ap-southeast-1.maas.aliyuncs.com/compatible-model/v1" # 你的专属亚太节点地址
)
...
if uploaded_files:
    ...
    col3.metric("客单价 (元)", f"{(df['消费金额'].sum() / len(df)):,.2f}")
    ...
    with chart_col2:
        st.write("🛍️ **商品类别营收占比**")
        category_sales = df.groupby('商品类别')['消费金额'].sum().sort_values(ascending=False)
        st.bar_chart(category_sales)
        
    st.write("🏙️ **TOP 用户城市分布**")
    ...
    if st.button("一键生成深度商业诊断"):
        with st.spinner("AI 正在深度思考业务逻辑..."):
            # 【这是大厂最核心的玩法】：把海量数据聚合成精简字符串，防超长，防幻觉！
            summary_text = f"""
            总体表现：总销售额 {df['消费金额'].sum():.2f} 元，共 {len(df)} 笔订单。
            品类表现摘要：\n{category_sales.to_string()}
            城市表现前五名：\n{city_sales.head(5).to_string()}
            男女消费占比：\n{df.groupby('用户性别')['消费金额'].sum().to_string()}
            """
            ...
            1. **大盘诊断**：评价目前的总体业绩与客单价情况。
            2. **爆款与品类洞察**：根据品类表现，指出我们的绝对核心品类是什么？哪些品类存在长尾拖累？
            3. **高价值人群画像**：从性别、城市维度，描绘出谁在为我们贡献最多的利润？
            4. **下一步行动建议**：给出至少 3 条落地性极强、针对特定人群或品类的营销/备货策略。
            """
